...
def test_fixed_to_variable_width(writable_file):
    ...
    # write T -> S
    # FIXME segfault
    # x[0] = np.asarray("baz", dtype="T")
    # np.testing.assert_array_equal(x[:], [b"baz", b"bar"])
    # np.testing.assert_array_equal(y[:], ["baz", "bar"])
...
def test_write_object_into_npystrings(writable_file):
    ...
    assert x.dtype.kind == "T"
    x[0] = np.asarray("bar", dtype="O")
    np.testing.assert_array_equal(x[:], "bar")
...
def test_write_npystrings_into_object(writable_file):
    ...
    np.testing.assert_array_equal(x[:], b"bar")
