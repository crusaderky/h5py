...
class TestContains(BaseGroup):
    ...
    def test_trailing_slash(self):
        ...
        self.f.create_group(name("g"))
        self.f[name("x")] = 42
        self.assertIn(f"/{name("g")}/", self.f)
        self.assertIn(f"{name("g")}/", self.f)
        self.assertIn(f"/{name("x")}/", self.f)
        self.assertIn(f"{name("x")}/", self.f)
    ...
    def test_oddball_paths(self):
        ...
        self.assertIn('././/', self.f)
        grp = self.f[name('x')]
        self.assertIn(f'.//{xyz}', self.f)
        self.assertNotIn(f'.//{xyz}', grp)
        self.assertIn(xyz, self.f)
        self.assertIn(f'./{xyz}', self.f)
...
class TestGet(BaseGroup):
    ...
    def test_get_class(self):
        ...
        out = self.f.get(name("x"), getclass=True)
        self.assertEqual(out, Group)

        self.f.create_dataset(name("y"), (4,))
        out = self.f.get(name("y"), getclass=True)
        self.assertEqual(out, Dataset)

        self.f[name("z")] = np.dtype('|S10')
        out = self.f.get(name("z"), getclass=True)
...
class TestCopy(TestCase):
    ...
    def test_copy_path_to_path(self):
        ...
        self.assertArrayEqual(baz[name("bar")], np.array([1,2,3]))
    ...
    def test_copy_dataset(self):
        ...
        self.assertArrayEqual(self.f1[f'/{name("grp")}/foo'], np.array([1,2,3]))
    ...
    def test_copy_shallow(self):
        ...
        self.assertIsInstance(self.f2[f'/{name("foo")}'], Group)
    ...
    def test_copy_soft_links(self):
        ...
        foo = self.f1.create_group(name('foo'))
        foo[name('baz')] = SoftLink(name('/bar'))
    ...
    def test_copy_refs(self):
        ...
        self.assertArrayEqual(self.f2[name('root') + '/foo'], np.array([1,2,3]))
        self.assertArrayEqual(self.f2[name('root') + '/bar'], np.array([4,5,6]))
        foo_bar = self.f2[name('root') + '/foo'].attrs['bar']
        self.assertArrayEqual(self.f2[foo_bar], np.array([4,5,6]))
        # There's only one copy of bar, which the reference points to.
        self.assertEqual(self.f2[foo_bar], self.f2[name('root') + '/bar'])
...
class TestMove(BaseGroup):
    ...
    def test_move_softlink(self):
        """ Moving a soft link """
        self.f[name("soft")] = h5py.SoftLink("relative/path")
